    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        public virtual DiscountValidationResult ValidateDiscount(DiscountForCaching discount, Customer customer, string[] couponCodesToValidate)
        {
            ...
            if (discount.RequiresCouponCode)
            {
                ...
                if (couponCodesToValidate.Length > 1)
                {

                    result.Errors = new List<string> { _localizationService.GetResource("ShoppingCart.Discount.MoreThanOnce") };
                    return result;
            ...
            //check minimum order
            if (discount.OrderAbove > 0 && discount.OrderAbove > price)
            {

                result.Errors = new List<string> { _localizationService.GetResource("ShoppingCart.Discount.MinimumOrder" + " " + discount.OrderAbove) };
                return result;
            }
                ...
                switch (discount.DiscountLimitation)
            {
                ...
                case DiscountLimitationType.NTimesDiscountAndNTimePerCustomer:
                    {
                        if (customer.IsRegistered())
                        {
                            //check customer used time
                            var usedTimes = GetAllDiscountUsageHistory(discount.Id, customer.Id, null, 0, 1).TotalCount;
                            if (usedTimes >= discount.LimitationTimesPerCustomer)
                            {
                                result.Errors = new List<string> { _localizationService.GetResource("ShoppingCart.Discount.CannotBeUsedAnymore") };
                                return result;
                            }
                            ...
                            //check total used time
                            var discountUsed = GetAllDiscountUsageHistory(discount.Id,null, null, 0, 1).TotalCount;
                            if (usedTimes >= discount.LimitationTimesPerTotal)
                            {
                                result.Errors = new List<string> { _localizationService.GetResource("ShoppingCart.Discount.CannotBeUsedAnymore") };
                                return result;
                            }
