...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        private readonly IRepository<CustomerPicture> _customerPictureRepository;
        ...
        public CustomerService(ICacheManager cacheManager,
            ...
            IRepository<CustomerPicture> customerPictureRepository,
            ...
            CommonSettings commonSettings)
        {
            ...
            this._customerPictureRepository = customerPictureRepository;
        }
        ...
        #region Customer Pictures

        public virtual CustomerPicture GetCustomerPictureById(int customerPictureId)
        {
            if (customerPictureId == 0)
                return null;

            return _customerPictureRepository.GetById(customerPictureId);
        }

        public virtual IPagedList<CustomerPicture> GetAllCustomerPictures(int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _customerPictureRepository.Table;
            query = query.OrderByDescending(c => c.UploadDateTimeUtc);

            var customersPictures = new PagedList<CustomerPicture>(query, pageIndex, pageSize);
            return customersPictures;
        }

        public virtual void DeleteCustomerPicture(CustomerPicture customerPicture)
        {
            if (customerPicture == null)
                throw new ArgumentNullException("customerPicture");

            _customerPictureRepository.Delete(customerPicture);

            //event notification
            _eventPublisher.EntityDeleted(customerPicture);
        }

        public virtual CustomerPicture GetCustomerPictureByPictureId(int customerPictureId)
        {
            if (customerPictureId == 0)
                return null;
            var query = from cp in _customerPictureRepository.Table
                        where cp.PictureId == customerPictureId
                        select cp;
            //return _customerPictureRepository.GetById(customerPictureId);
            return query.FirstOrDefault();
        }

        public virtual void UpdateCustomerPicture(CustomerPicture customerPicture)
        {
            if (customerPicture == null)
                throw new ArgumentNullException("customerPicture");

            _customerPictureRepository.Update(customerPicture);

            //event notification
            _eventPublisher.EntityUpdated(customerPicture);
        }

        public virtual void InsertCustomerPicture(CustomerPicture customerPicture)
        {
            if (customerPicture == null)
                throw new ArgumentNullException("customerPicture");

            _customerPictureRepository.Insert(customerPicture);

            //event notification
            _eventPublisher.EntityInserted(customerPicture);
        }
        #endregion
