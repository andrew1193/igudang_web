        ...
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password)
        {
            ...
            if (customer.IsVendor())
            {
                ...
                if (ev != null)
                {
                    ...
                    DateTime end = ev.EndedOnUtc;
                    var strExpiredDay = _settingService.GetSettingByKey<string>("vendorloginsettings.expireddays");
                    double days = double.Parse(strExpiredDay);
                    DateTime actualEndDate = end.AddDays(days);
                    if (now > actualEndDate)
                    {
                        return CustomerLoginResults.Expired;
                    }
                }
            }
