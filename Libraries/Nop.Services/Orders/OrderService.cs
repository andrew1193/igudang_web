    ...
    public partial class OrderService : IOrderService
    {
            ...
            string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (vendorId > 0)
            {
                ...
                query = query.Where(o => o.PaymentStatusId == 30);
            }
            if (customerId > 0)
        ...
        public IEnumerable<Order> GetOrderByStatusAndPaymentStatus(int orderStatusId, int paymenStatusId)
         {
            if (orderStatusId == 0 && paymenStatusId ==0)
                return null;


            var datetimeNow = DateTime.Now.ToUniversalTime().AddMinutes(-8);
            var list = _orderRepository.Table.Where(x => x.OrderStatusId == orderStatusId && x.PaymentStatusId == paymenStatusId  && x.CreatedOnUtc <= datetimeNow).ToList();
