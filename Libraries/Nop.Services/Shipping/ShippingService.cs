...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        public void SetDefaultShippingMethodToCustomer(IList<ShoppingCartItem> cart, string shippingName, string shippingMethod, Customer customer)
        {
            try
            {
                ...
                //    throw new Exception("Selected shipping method can't be parsed");
                string selectedName = shippingName;
                string shippingRateComputationMethodSystemName = shippingMethod;
                ...
                if (shippingOptions == null || !shippingOptions.Any())
                {
                    ...
                    //{
                    //    shipAddress = _workContext.CurrentCustomer.ShippingAddress;
                    //}
                    if(_workContext.CurrentCustomer.ShippingAddress != null)
                    {
                        shippingOptions = GetShippingOptions(cart, _workContext.CurrentCustomer.ShippingAddress,
                    ...
                    {
                        var newAddress = _addressService.GetAddressById(GlobalValue.DefaultAddress.DefaultAddressId);
                        shippingOptions = GetShippingOptions(cart, newAddress,
                       _workContext.CurrentCustomer, shippingRateComputationMethodSystemName, _storeContext.CurrentStore.Id).ShippingOptions.ToList();
                    }
                }
                else
