    ...
    public partial class ProductService : IProductService
    {
            ...
            bool? overridePublished = null)
        {
            ...
            else
            {
                ...
                var nowUtc = _dateTimeHelper.ConvertToUserTime(DateTime.UtcNow, DateTimeKind.Utc);
                if (markedAsNewOnly)
                {
        ...
        //    IList<int> filterableSpecificationAttributeOptionIds;
        //    return SearchProducts(out filterableSpecificationAttributeOptionIds, false,
        //        pageIndex, pageSize, categoryIds,eventIds, manufacturerId,
        //        storeId, vendorId, warehouseId,
        //        productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts,
        //        priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku,
        //        searchProductTags, languageId, filteredSpecs, 
        ...
        //    //validate "eventIds" parameter
        //    if (eventIds != null && eventIds.Contains(0))
        //        eventIds.Remove(0);
        ...
        //        {
        //            query = from p in query
        //                from pc in p.ProductCategories.Where(pc => eventIds.Contains(pc.CategoryId))
        //                where (!featuredProducts.HasValue || featuredProducts.Value == pc.IsFeaturedProduct)
        //                select p;
        //        }
        ...
        public IList<Product> GetProductByVendorId(int vendorId)
        {
            var products = _productRepository.Table.ToList();
            products = products.Where(x => x.Deleted == false && x.VendorId == vendorId).ToList();
            return products;
        }
        ...
        {
            var products = _productRepository.Table.ToList();
            products = products.Where(x => x.Deleted == false).ToList();
            return products;
        }
