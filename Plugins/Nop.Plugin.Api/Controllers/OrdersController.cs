...
namespace Nop.Plugin.Api.Controllers
{
    ...
    public class OrdersController : BaseApiController
    {
        ...
        public IHttpActionResult CreateOrder([ModelBinder(typeof(JsonModelBinder<OrderDto>))] Delta<OrderDto> orderDelta)
        {
            ...
            string key = orderDelta.Dto.CustomerId.ToString() + "_" + orderDelta.Dto.PaymentMethodSystemName + "_" + orderDelta.Dto.ShippingMethod + "_" + orderDelta.Dto.BillingAddress.Id.ToString() + "_" + orderDelta.Dto.ShippingAddress.Id.ToString() + "_CreateOrder";
            var strMd5 = CommonHelper.CalculateMD5Hash(key);
            if (strMd5 != orderDelta.Dto.ApiKey)
            {
        ...
        [HttpGet]
        public IHttpActionResult GetAllShippingMethod()
        {
            IList<ShippingMethod> shipping_methods = _orderApiService.GetAllShippingMethod();

            IList<ShippingMethodsDto> shippingMethodsAsDtos = shipping_methods.Select(x => _dtoHelper.PrepareShippingMethodsDTO(x)).ToList();

            var shippingMethodRootObject = new ShippingMethodsRootObject()
            {
                ShippingMethods = shippingMethodsAsDtos
            };

            var json = _jsonFieldsSerializer.Serialize(shippingMethodRootObject, null);
