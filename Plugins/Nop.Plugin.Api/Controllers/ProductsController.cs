...
namespace Nop.Plugin.Api.Controllers
{
    ...
    public class ProductsController : BaseApiController
    {
        ...
        public ProductsController(IProductApiService productApiService,
                                  ...
                                  IDateTimeHelper datetimeHelper) : base(jsonFieldsSerializer, aclService, customerService, storeMappingService, storeService, discountService, customerActivityService, localizationService, pictureService)
        {
        ...
        public IHttpActionResult GetProducts(ProductsParametersModel parameters)
        {
            ...
            if (ev.Id > 0)
            {
                startDate = _datetimeHelper.ConvertToUtcTime(ev.StartedOnUtc, DateTimeKind.Local);
                endDate = _datetimeHelper.ConvertToUtcTime(ev.EndedOnUtc, DateTimeKind.Local);
            }
            ...
            IList<Product> allProducts = _productApiService.GetProducts(parameters.Ids, parameters.CreatedAtMin, parameters.CreatedAtMax, parameters.UpdatedAtMin,
                                                                        ...
                                                                        parameters.VendorName, parameters.ProductName, parameters.CategoryName, true, startDate, endDate);
        ...
        public IHttpActionResult GetProductsCount(ProductsCountParametersModel parameters)
        {
            ...
            int allProductsCount = _productApiService.GetProductsCount(parameters.CreatedAtMin, parameters.CreatedAtMax, parameters.UpdatedAtMin,
                                                                       ...
                                                                       parameters.ProductName, parameters.CategoryName, parameters.CategoryId, startDate, endDate);

            var productCountDto = new ProductCountDto();
            productCountDto.Count = allProductsCount;

            var productsCountRootObject = new ProductsCountRootObject()
            {
                Count = productCountDto
            };

            var json = _jsonFieldsSerializer.Serialize(productsCountRootObject, "");
        ...
        public IHttpActionResult GetProductById(int id, string fields = "")
        {
            ...
            if (ev.Id > 0)
            {
                start = _datetimeHelper.ConvertToUtcTime(ev.StartedOnUtc, DateTimeKind.Local);
                end = _datetimeHelper.ConvertToUtcTime(ev.EndedOnUtc, DateTimeKind.Local);
            }

            Product product = _productApiService.GetProductByIdAndEventDate(id, start, end);
            ...
            if (product.AvailableStartDateTimeUtc == null && product.AvailableEndDateTimeUtc == null)
            {
                return Error(HttpStatusCode.NotFound, "product", "not found");
            }
        ...
        [HttpGet]
        [ResponseType(typeof(ProductReviewsRootObject))]
        [GetRequestsErrorInterceptorActionFilter]
        public IHttpActionResult GetProductReviewsByProductId(ProductsParametersModel parameters)
        {
            try
            {
                if (parameters.ProductId <= 0)
                {
                    return Error(HttpStatusCode.BadRequest, "product id", "invalid product id");
                }

                var productReviews = _productApiService.GetProductReviews(parameters.ProductId);


                IList<ProductReviewDto> productReviewsAsDtos = new List<ProductReviewDto>();

                if (productReviews != null)
                {
                    productReviewsAsDtos = productReviews.Select(productreview =>
                    {
                        return _dtoHelper.PrepareProductReviewDto(productreview);
                ...
                var json = _jsonFieldsSerializer.Serialize(productReviewsRootObject, parameters.Fields);
        ...
        [HttpGet]
        [ResponseType(typeof(ProductRatingRootObject))]
        [GetRequestsErrorInterceptorActionFilter]
        public IHttpActionResult GetProductRatingByProductId(ProductsParametersModel parameters)
        {
            try
            {
                if (parameters.ProductId <= 0)
                ...
                var product = _productService.GetProductById(parameters.ProductId);

                var productReviewOverviewModel = _dtoHelper.PrepareProductReviewOverviewModel(product);

                ProductRatingDto productRatingAsDto = new ProductRatingDto();
                productRatingAsDto.ProductId = productReviewOverviewModel.ProductId;
                productRatingAsDto.RatingSum = productReviewOverviewModel.RatingSum;
                productRatingAsDto.TotalReviews = productReviewOverviewModel.TotalReviews;
                productRatingAsDto.AllowCustomerReviews = productReviewOverviewModel.AllowCustomerReviews;
                productRatingAsDto.RatingPercent = ((productReviewOverviewModel.RatingSum * 100) / productReviewOverviewModel.TotalReviews) / 5;

                var productRatingRootObject = new ProductRatingRootObject()
                {
                    ProductRating = productRatingAsDto
                };

                var json = _jsonFieldsSerializer.Serialize(productRatingRootObject, parameters.Fields);
