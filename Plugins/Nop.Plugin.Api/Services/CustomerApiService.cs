...
namespace Nop.Plugin.Api.Services
{
    public class CustomerApiService : ICustomerApiService
    {
        ...
        public CustomerDto GetCustomerById(int id, bool showDeleted = false)
        {
            ...
            var roles = _customerRepository.GetById(id).CustomerRoles;
            var roleIds = new List<int>();
            if (roles.Count() > 0)
            {
                foreach (var item_roles in roles)
                {
                    roleIds.Add(item_roles.Id);
                    //if (item_roles.Id == GlobalValue.DefaultCustomerRoleIds.Registered)
                    //{
                    //    roleIds.Add(item_roles.Id);
                    //}
                }
        ...
        public Customer GetGuestCustomer(string query = "")
        {
            var customer = new Customer();
            if (query != "")
            {
                customer = _customerRepository.Table.Where(x => x.LastIpAddress == query && x.CustomerRoles.Any(y => y.Id == GlobalValue.DefaultCustomerRoleIds.Guest)).FirstOrDefault();
                //customers = customers.Where(x => x.LastIpAddress == query && x.CustomerRoles.Any(y => y.Id == GlobalValue.DefaultCustomerRoleIds.Guest)).FirstOrDefault();
            }
        ...
        {
            bool exist = false;
            var customers = _customerRepository.Table.Where(x => x.Deleted == false);
            exist = customers.Any(x => x.Email == email);
            return exist;
        }

        public bool CheckIfEmailExistForUpdate(string email, int oriCustId)
        {
            bool exist = false;
            var customers = _customerRepository.Table.Where(x => x.Deleted == false && x.Id != oriCustId);
            exist = customers.Any(x => x.Email == email);
            return exist;
        }
    }
