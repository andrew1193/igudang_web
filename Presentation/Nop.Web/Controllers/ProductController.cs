...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual ActionResult ProductDetails(int productId, int updatecartitemid = 0, string err = null)
        {
        ...
        [ChildActionOnly]
        public virtual ActionResult ProductsAlsoPurchased2(int productId, int? productThumbPictureSize)
        {
            if (!_catalogSettings.ProductsAlsoPurchasedEnabled)
                return Content("");
            ...
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
            //availability dates
            products = products.Where(p => p.IsAvailable()).Take(4).ToList();
        ...
        [ChildActionOnly]
        public virtual ActionResult HomepageEventBannerProducts(int id)
        {
            var products = _productService.GetAllProducts();
            var vendors = _vendorService.GetVendorByEventId(id).Select(x => x.Id);
            var productList = new List<Product>();
            if (vendors.Count() > 0)
            {
                foreach(var item in vendors)
                {
                    productList.AddRange(products.Where(x => x.VendorId == item));
                }
            }

            productList = productList.Where(x => x.ShowOnHomePage == true).OrderBy(x => x.DisplayOrder).Take(3).ToList();
            var model = _productModelFactory.PrepareProductOverviewModels(productList, true, true, null).ToList();
            return PartialView(model);
        }
...

        {
            ...
            products = products.Where(x => x.AvailableStartDateTimeUtc != null && x.AvailableEndDateTimeUtc != null).ToList();
            var ev = _eventService.GetCurrentEvent();
            if(ev != null)
            {
                products = products.Where(x => x.AvailableStartDateTimeUtc >= ev.StartedOnUtc && x.AvailableEndDateTimeUtc <= ev.EndedOnUtc).ToList();
            }
        ...
        //[ChildActionOnly]
        public virtual ActionResult AddReviewPanel(int productId)
        {
            var product = _productService.GetProductById(productId);
            ...
            var rateNums = new List<SelectListItem>();
            rateNums.Add(new SelectListItem { Text = "-- Select One --", Value = "" });
            rateNums.Add(new SelectListItem { Text = "1", Value = "1" });
            rateNums.Add(new SelectListItem { Text = "2", Value = "2" });
            rateNums.Add(new SelectListItem { Text = "3", Value = "3" });
            rateNums.Add(new SelectListItem { Text = "4", Value = "4" });
            rateNums.Add(new SelectListItem { Text = "5", Value = "5" });
            model.AddProductReview.RateNums = rateNums;
            return PartialView(model);
        }
        ...
        [ChildActionOnly]
        public virtual ActionResult ProductReviewTab(int productId)
        {
            var product = _productService.GetProductById(productId);
            //if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
            //    return RedirectToRoute("HomePage");
        ...
        [ChildActionOnly]
        public virtual ActionResult _AddReviewPanel(int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
                return RedirectToRoute("HomePage");
        ...
        [ChildActionOnly]
        public virtual ActionResult _ProductReviewTab(int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
                return RedirectToRoute("HomePage");
        ...
        [PublicAntiForgery]
        [CaptchaValidator]
        public virtual ActionResult AddReviewFromPanel(string productId, ProductReviewsModel model, bool captchaValid, string currentUrl)
        {
            var product = _productService.GetProductById(int.Parse(productId));
            if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
                return RedirectToRoute("HomePage");
            ...
            if (_catalogSettings.ProductReviewPossibleOnlyAfterPurchasing &&
                !_orderService.SearchOrders(customerId: _workContext.CurrentCustomer.Id, productId: int.Parse(productId), osIds: new List<int> { (int)OrderStatus.Complete }).Any())
                ModelState.AddModelError(string.Empty, _localizationService.GetResource("Reviews.ProductReviewPossibleOnlyAfterPurchasing"));
            ...
            //return RedirectToAction("ProductDetails", new { productId = productId, err = "review" });
            return Redirect(currentUrl);
        }
