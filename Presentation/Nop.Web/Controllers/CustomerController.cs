...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual ActionResult Login(LoginModel model, string returnUrl, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (loginResult)
                {
                    ...
                    case CustomerLoginResults.Expired:
                        ModelState.AddModelError("", _localizationService.GetResource("Account.Login.WrongCredentials.LoginExpired"));
                        break;
        ...
        [NopHttpsRequirement(SslRequirement.Yes)]
        public virtual ActionResult AddressAdd(string billOrShip)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();

            var newAddress = new Address();
            newAddress.CountryId = 53;
            ...
            if (billOrShip != null)
            {
                model.billingOrShipping = billOrShip;
            }
        ...
        [PublicAntiForgery]
        [ValidateInput(false)]
        public virtual ActionResult AddressAdd(CustomerAddressEditModel model, FormCollection form, string hfBillOrShip)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            if (ModelState.IsValid)
            {
                if (hfBillOrShip == "bill")
                {
                    var billAddress = model.Address.ToEntity();
                    billAddress.CustomAttributes = customAttributes;
                    billAddress.CreatedOnUtc = DateTime.UtcNow;
                    if (billAddress.CountryId == 0)
                        billAddress.CountryId = null;
                    if (billAddress.StateProvinceId == 0)
                        billAddress.StateProvinceId = null;
                    _addressService.InsertAddress(billAddress);
                    customer.BillingAddress = billAddress;
                    _customerService.UpdateCustomer(customer);
                }
                else if (hfBillOrShip == "ship")
                {
                    var shipAddress = model.Address.ToEntity();
                    shipAddress.CustomAttributes = customAttributes;
                    shipAddress.CreatedOnUtc = DateTime.UtcNow;
                    if (shipAddress.CountryId == 0)
                        shipAddress.CountryId = null;
                    if (shipAddress.StateProvinceId == 0)
                        shipAddress.StateProvinceId = null;
                    _addressService.InsertAddress(shipAddress);
        ...
        #region My account / Photo Booth

        [NopHttpsRequirement(SslRequirement.Yes)]
        public virtual ActionResult PhotoBooth()
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            var model = new CustomerPhotoBoothModel();
            model = _customerModelFactory.PrepareCustomerPhotoBoothModel(model);
            return View(model);
        }

        [HttpPost, ActionName("PhotoBooth")]
        [PublicAntiForgery]
        [FormValueRequired("upload-photobooth")]
        public virtual ActionResult UploadPhotoBooth(CustomerPhotoBoothModel model, HttpPostedFileBase uploadedFile)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var customerPhotoBooth = _pictureService.GetPictureById(customer.GetAttribute<int>(SystemCustomerAttributeNames.PhotoBoothPictureId));
                    if ((uploadedFile != null) && (!String.IsNullOrEmpty(uploadedFile.FileName)))
                    {
                        int photoMaxSize = _customerSettings.PhotoBoothMaximumSizeBytes;
                        if (uploadedFile.ContentLength > photoMaxSize)
                            throw new NopException(string.Format(_localizationService.GetResource("Account.PhotoBooth.MaximumUploadedFileSize"), photoMaxSize));

                        byte[] customerPictureBinary = uploadedFile.GetPictureBits();
                        if (customerPhotoBooth != null)
                        {
                            customerPhotoBooth = _pictureService.UpdatePicture(customerPhotoBooth.Id, customerPictureBinary, uploadedFile.ContentType, null);
                            var customerPicture = _customerService.GetCustomerPictureByPictureId(customerPhotoBooth.Id);
                            if (customerPicture != null)
                            {
                                customerPicture.PictureId = customerPhotoBooth.Id;
                                customerPicture.UploadDateTimeUtc = DateTime.Now;
                                _customerService.UpdateCustomerPicture(customerPicture);
                            }
                            else
                            {
                                _customerService.InsertCustomerPicture(new CustomerPicture
                                {
                                    PictureId = customerPhotoBooth.Id,
                                    CustomerId = customer.Id,
                                    Published = false,
                                    UploadDateTimeUtc = DateTime.Now
                                });
                            }
                        ...
                        {
                            customerPhotoBooth = _pictureService.InsertPicture(customerPictureBinary, uploadedFile.ContentType, null);
                            _customerService.InsertCustomerPicture(new CustomerPicture
                            {
                                PictureId = customerPhotoBooth.Id,
                                CustomerId = customer.Id,
                    ...
                    int customerPhotoBoothId = 0;
                    if (customerPhotoBooth != null)
                        customerPhotoBoothId = customerPhotoBooth.Id;

                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PhotoBoothPictureId, customerPhotoBoothId);

                    model.PhotoUrl = _pictureService.GetPictureUrl(
                        customer.GetAttribute<int>(SystemCustomerAttributeNames.PhotoBoothPictureId),
                        _mediaSettings.PhotoBoothPictureSize,
                        false);
                    return View(model);
                }
                catch (Exception exc)
        ...
        [HttpPost, ActionName("PhotoBooth")]
        [PublicAntiForgery]
        [FormValueRequired("remove-photobooth")]
        public virtual ActionResult RemovePhotoBooth(CustomerPhotoBoothModel model, HttpPostedFileBase uploadedFile)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            var customerPhotoBooth = _pictureService.GetPictureById(customer.GetAttribute<int>(SystemCustomerAttributeNames.PhotoBoothPictureId));
            if (customerPhotoBooth != null)
                _pictureService.DeletePicture(customerPhotoBooth);
            var customerPicture = _customerService.GetCustomerPictureByPictureId(customerPhotoBooth.Id);
            if (customerPicture != null)
                _customerService.DeleteCustomerPicture(customerPicture);
            _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PhotoBoothPictureId, 0);

            return RedirectToRoute("CustomerPhotoBooth");
        }
