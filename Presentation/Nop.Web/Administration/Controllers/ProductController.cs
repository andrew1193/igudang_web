...
namespace Nop.Admin.Controllers
{
    ...
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void PrepareProductModel(ProductModel model, Product product, bool setPredefinedValues, bool excludeProperties)
        {
            ...
            //****check expir date to show addnew button
            model.ExpireDate = false;
            if (_workContext.CurrentVendor != null)
            {
                var currentVendorName = _workContext.CurrentVendor.Id;

                if (currentVendorName != 0)
                {




                    var Vendorquery = _vendorRepository.Table;
                    var Eventquery = _eventRepository.Table;
                    var L2EQuery = (from st in Eventquery
                                    join vs in Vendorquery on st.Id equals vs.EventId
                                    where vs.Id == currentVendorName
                                    select new EndStartDate { EndDate = st.EndedOnUtc, StartDate = st.StartedOnUtc }).FirstOrDefault();





                    if (DateTime.Now > L2EQuery.EndDate || DateTime.Now < L2EQuery.StartDate)
                    {
                        model.ExpireDate = true;
                    }
            ...
            //****end check expir date to show addnew button/
        ...
        public ActionResult TriggerEvent(int id)
        {
            string result = "";
            if (id > 0)
            {
                ...
                if (ev != null)
                {
                    var start = _dateTimeHelper.ConvertToUserTime(ev.StartedOnUtc, DateTimeKind.Utc);
                    var end = _dateTimeHelper.ConvertToUserTime(ev.EndedOnUtc, DateTimeKind.Utc);
                    result = ev.Name + "," + start.ToString() + "," + end.ToString();
                }
            }
            return Json(result);
        }
        ...
        public virtual ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                product.IsBestSeller = model.MarkAsBestSeller;
                ...
                if (product.VendorId > 0)
                {
                    ...
                    if (vendor.EventId > 0)
                    {
                        ...
                        if (ev.Id > 0)
                        {
                            product.AvailableStartDateTimeUtc = ev.StartedOnUtc;
                            product.AvailableEndDateTimeUtc = ev.EndedOnUtc;
                        }
                    }
                }
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            model.MarkAsBestSeller = product.IsBestSeller;
            if (product.VendorId > 0)
            {
                var vendor = _vendorService.GetVendorById(product.VendorId);
                var ev = _eventService.GetEventById(vendor.EventId);
                if (ev != null)
                {
                    ...
                    model.AvailableStartDateTimeUtc = _dateTimeHelper.ConvertToUserTime(ev.StartedOnUtc, DateTimeKind.Utc);
                    model.AvailableEndDateTimeUtc = _dateTimeHelper.ConvertToUserTime(ev.EndedOnUtc, DateTimeKind.Utc);
                }
            }
        ...
        public virtual ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //if (!string.IsNullOrEmpty(hfStartDate))
                //{
                //    product.AvailableStartDateTimeUtc = DateTime.Parse(hfStartDate);
                //}
                //if (!string.IsNullOrEmpty(hfEndDate))
                //{
                //    product.AvailableEndDateTimeUtc = DateTime.Parse(hfEndDate);
                //}
                _productService.UpdateProduct(product);
                //search engine name
                model.SeName = product.ValidateSeName(model.SeName, product.Name, true);
