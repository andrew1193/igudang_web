...
namespace Nop.Admin.Controllers
{
    public partial class EventController : BaseAdminController
    {
        ...
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageEvents))
        ...
        public virtual ActionResult Create()
        {
            ...
            model.AvailableStartDateTimeUtc = DateTime.Now;
            model.AvailableEndDateTimeUtc = DateTime.Now;
        ...
        public virtual ActionResult Create(EventModel model, bool continueEditing)
        {
            ...
            //if (model.Published)
            //{
            //    var result = _eventService.CheckIsOtherEventActive();
            //    if (result)
            //    {
            //        ModelState.AddModelError("error", "There is an event marked as active.");
            //    }
            //}
            ...
            if (ModelState.IsValid)
            {
                ...
                ev.StartedOnUtc = _datetimeHelper.ConvertToUtcTime(model.AvailableStartDateTimeUtc, DateTimeKind.Local);
                ev.EndedOnUtc = _datetimeHelper.ConvertToUtcTime(model.AvailableEndDateTimeUtc, DateTimeKind.Local);
        ...
        public virtual ActionResult Edit(EventModel model, bool continueEditing)
        {
            ...
            //    if (result)
            //    {
            //        ModelState.AddModelError("error", "Cannot publish! There is an event marked as active.");
            //    }
            //}
            ...
            if (ModelState.IsValid)
            {
                ...
                var vendors = _vendorService.GetVendorByEventId(ev.Id);
                if (vendors.Count() > 0)
                {
                    foreach (var item in vendors)
                    {
                        UpdateProductStartEndDate(item.Id, ev);
                    }
                }
        ...
        private void UpdateProductStartEndDate(int vendorId, Event ev)
        {
            var products = _productService.GetProductByVendorId(vendorId);
            if (products.Count() > 0)
            {
                foreach (var item in products)
                {
                    //item.AvailableStartDateTimeUtc = ev.StartedOnUtc;
                    //item.AvailableEndDateTimeUtc = ev.EndedOnUtc;
                    item.AvailableStartDateTimeUtc = ev.StartedOnUtc;
                    item.AvailableEndDateTimeUtc = ev.EndedOnUtc;
