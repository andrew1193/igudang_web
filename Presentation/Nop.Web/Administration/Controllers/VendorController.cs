...
namespace Nop.Admin.Controllers
{
    public partial class VendorController : BaseAdminController
    {
        ...
        [NonAction]
        protected virtual void PrepareEventsModel(VendorModel model)
        {
            if (model == null)
            ...
            {
                Text = _localizationService.GetResource("Admin.Customers.Customers.Fields.Event.None"),
                Value = "0"
            });
            var events = SelectListHelper.GetEventList(_eventService, _cacheManager, true);
            foreach (var e in events)
                model.AvailableEvents.Add(e);
        }
        ...
        public virtual ActionResult Create(VendorModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //generate shipper id
                if (string.IsNullOrEmpty(vendor.ShipperId))
                {
                    vendor.ShipperId = GeneratesShipperId(vendor.Id);
                }
        ...
        private string GeneratesShipperId(int id)
        {
            string prefix = "000" + id.ToString();
            return prefix;
        }
