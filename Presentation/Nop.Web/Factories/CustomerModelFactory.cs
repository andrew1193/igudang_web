...
namespace Nop.Web.Factories
{
    ...
    public partial class CustomerModelFactory : ICustomerModelFactory
    {
        ...
        public virtual CustomerNavigationModel PrepareCustomerNavigationModel(int selectedTabId = 0)
        {
            ...
            //model.CustomerNavigationItems.Add(new CustomerNavigationItemModel
            //{
            //    RouteName = "CustomerPhotoBooth",
            //    Title = _localizationService.GetResource("Account.PhotoBooth"),
            //    Tab = CustomerNavigationEnum.PhotoBooth,
            //    ItemClass = "customer-avatar"
            //});
        ...
        public virtual CustomerAddressListModel PrepareCustomerAddressListModel()
        {
            ...
            if (_workContext.CurrentCustomer.BillingAddress != null)
            {
                ...
                var billAddressModel = new AddressModel();
                _addressModelFactory.PrepareAddressModel(billAddressModel, address: billAddress, excludeProperties: false, addressSettings: _addressSettings, loadCountries: () => _countryService.GetAllCountries(_workContext.WorkingLanguage.Id));
                model.BillingAddress = billAddressModel;
            }

            if (_workContext.CurrentCustomer.ShippingAddress != null)
            {
                //get shipping address
                var shipAddress = _workContext.CurrentCustomer.ShippingAddress;
                var shipAddressModel = new AddressModel();
                _addressModelFactory.PrepareAddressModel(shipAddressModel, address: shipAddress, excludeProperties: false, addressSettings: _addressSettings, loadCountries: () => _countryService.GetAllCountries(_workContext.WorkingLanguage.Id));
                model.ShippingAddress = shipAddressModel;
            }
        ...
        /// <summary>
        /// Prepare the customer photo booth model
        /// </summary>
        /// <param name="model">Customer photo booth model</param>
        /// <returns>Customer photo booth model</returns>
        public virtual CustomerPhotoBoothModel PrepareCustomerPhotoBoothModel(CustomerPhotoBoothModel model)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            model.PhotoUrl = _pictureService.GetPictureUrl(
                _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.PhotoBoothPictureId),
